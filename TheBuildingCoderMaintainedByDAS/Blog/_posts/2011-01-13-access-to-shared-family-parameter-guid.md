---
layout: "post"
title: "Access to Shared Family Parameter GUID"
date: "2011-01-13 05:00:00"
...
categories:
  ...
  - "Family"
  - "Parameters"
original_url: "https://thebuildingcoder.typepad.com/blog/2011/01/access-to-shared-family-parameter-guid.html "
typepad_basename: "access-to-shared-family-parameter-guid"
typepad_status: "Publish"
---

<p>Here is a question that came up several times. 
It also demonstrates a neat (if somewhat time consuming) use of Reflection to access a property that 
...
If it is a shared parameter, how can I retrieve its original GUID as it was when it was originally 

added to the family? 

<p>Note that I need the <strong>original</strong> GUID used to load that parameter, and cannot rely on 

the current shared parameters file, which may not be the same one as originally used.

<p>I tried to solve this directly, but retrieving the parameter definition of a parameter on a 

family only reports it as being an InternalDefinition, even if it is actually a shared parameter, which 

should return an ExternalDefinition instead. 
I tested it running a piece of code like this: 

<pre class="code">
&nbsp; <span class="teal">FamilyParameter</span> parameter 
&nbsp; &nbsp; = &lt;get a family parameter 
&nbsp; &nbsp; &nbsp; &nbsp; <span class="blue">ref</span> from the <span 

class="teal">FamilyManager</span>&gt; 
&nbsp;
&nbsp; <span class="teal">ExternalDefinition</span> externalDef 
&nbsp; &nbsp; = parameter.Definition <span class="blue">as</span> <span 

class="teal">ExternalDefinition</span>; 
&nbsp;
&nbsp; <span class="teal">InternalDefinition</span> internalDef 
&nbsp; &nbsp; = parameter.Definition <span class="blue">as</span> <span 

class="teal">InternalDefinition</span>; 
</pre>
...
something called "m_Parameter" which <strong>does</strong> have the properties IsShared and GUID which 

<strong>do</strong> have the correct values. 

<p>Unfortunately, again, you cannot simply cast an Autodesk.Revit.DB.FamilyParameter to an 

Autodesk.Revit.DB.Parameter type. 
...
It retrieves the underlying family parameter definition IsShared and GUID properties and returns true 

if the family parameter is shared and has a GUID:
...
<p>I implemented a new external command CmdFamilyParamGuid in The Building Coder samples to test this 
...
<span class="asset  asset-generic at-xid-6a00e553e1689788330147e187ea39970b"><a href="http://thebuildingcoder.typepad.com/files/bc_11_82-1.zip">version 2011.0.82.0</a></span>

of The Building Coder samples including the complete source code and Visual Studio solution with the 
