...
date: "2008-09-08 14:00:00"
author: "Jeremy Tammik"
categories:
  - "Debugging"
  - "Getting Started"
original_url: "https://thebuildingcoder.typepad.com/blog/2008/09/debugging-a-rev.html "
typepad_basename: "debugging-a-rev"
...
<p>For completeness sake, and to ensure that we are really all on the same page, I suggest we start from scratch with a brand new Revit add-in project. That means that we will be going through the same steps as those described in the DevTV presentation and webcast recording mentioned in the <a href="http://thebuildingcoder.typepad.com/blog/2008/08/getting-started.html">getting started</a> post. Here is the short version of the description to set up a new Revit add-in:</p>

<ul><li>Start Visual Studio 2005.</li>
...
<li>Rename the class from Class1 to Command, simply to follow a common Revit add-in practice. I do this by clicking on the file name in the solution explorer and renaming that. It automatically renames the class contained in it as well.</li></ul>

<p>From here on, there are some small differences between C# and VB. For C#, we can proceed as follows:</p>

<ul><li>Remove the unneeded references to System.Data and System.Xml.</li>
...
<li>Do not forget to set the 'Copy Local' flag to False. You do so by opening the properties of RevitAPI.dll. See <a href="http://thebuildingcoder.typepad.com/blog/2008/09/the-sdk-samples.html">The SDK Samples Solution</a> for a more detailed explanation of this step.</li>
...
<li>Replace the automatically generated exception throwing code by a valid return value, e.g. &quot;return IExternalCommand.Result.Succeeded;&quot;</li></ul>
...
<ul><li>Add a reference to RevitAPI.dll by opening the project, selecting the References tab, and clicking Add...</li>

<li>Do not forget to set the 'Copy Local' flag to False. You do so by opening the properties of RevitAPI.dll. See <a href="06">The SDK Samples Solution</a> for a more detailed explanation of this step.</li>
...
<li>Implement the Execute() method. In VB, you do not have the 'Implement Interface' option provided for C#, but you can go to the definition of IExternalCommand and copy the code from there.</li></ul>
